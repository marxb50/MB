    ...
    <div className="min-h-screen bg-black text-gray-200">
      <header className="bg-gray-900 border-b border-gray-700">
        <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-white">Painel do Fiscal</h1>
        ...
        <div className="bg-gray-800 p-4 sm:p-6 rounded-lg">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
             ...
             <div className="flex items-center gap-2 sm:gap-4 w-full sm:w-auto">
                 ...
                 <button onClick={fetchSubmissions} disabled={loading || isBatchUpdating} className="p-2 rounded-md bg-gray-700 hover:bg-gray-600 disabled:opacity-50 transition-colors">
                ...
                <thead className="bg-gray-700/50">
                  <tr>
                    <th scope="col" className="px-6 py-3">
                       <input 
                            type="checkbox" 
                            className="h-4 w-4 text-green-600 bg-gray-600 border-gray-500 rounded focus:ring-green-500"
                    ...
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Localização</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Serviço</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Foto</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Status</th>
                  </tr>
                </thead>
                <tbody className="bg-gray-800 divide-y divide-gray-700">
                  {submissions.map((s) => (
                    <tr key={s.id} className={`${selectedIds.has(s.id) ? 'bg-green-900/50' : ''} hover:bg-gray-700/50 transition-colors duration-150`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-400">{new Date(s.dataHora).toLocaleDateString()}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{s.nome}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-green-400 hover:underline">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{s.servico}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-green-400 hover:underline">
                        <a href={s.foto_inicial_URL} target="_blank" rel="noopener noreferrer">Ver foto</a>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          s.status === ServiceStatus.ENVIADO_AO_FISCAL ? 'bg-yellow-900 text-yellow-300' : 
                          s.status === ServiceStatus.ENVIADO_A_EMPRESA ? 'bg-blue-900 text-blue-300' : 
